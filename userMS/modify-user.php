...
<?php
    ...
    //get the previous user enter data to this page
    if(isset($_GET['user_id'])){
        //getting user information 

        $user_id = mysqli_real_escape_string($connection , $_GET['user_id']);//sanitize the user id 
        $query = "SELECT *  FROM user WHERE id = {$user_id} LIMIT 1";//command to get the user id value
        $result_set = mysqli_query($connection , $query);//match the value from the database 

        if($result_set){
            if(mysqli_num_rows($result_set) == 1){
                //user found
                $result = mysqli_fetch_assoc($result_set);//retrive the data
                $first_name = $result['first_name'];
                $last_name = $result['last_name'];
                $email = $result['email'];
            }else{
                //user not found
                header('Location : users.php?err=usernot_found');
